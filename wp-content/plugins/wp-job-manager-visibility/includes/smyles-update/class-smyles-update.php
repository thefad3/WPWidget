...
class sMyles_JMV_Update {
	...
	public function action_links( $links ) {
		...
		$links[] = '<a href="' . esc_url_raw( remove_query_arg( array( 'deactivated_licence', 'activated_licence' ), add_query_arg( $this->plugin_slug . '_deactivate_licence', 1 ) ) ) . '">' . __( 'Deactivate Licence', 'wp-job-manager-visibility' ) . '</a>';
	...
	public function activated_key_notice() {
		...
		?>
		<div class="updated">
		<p><?php printf( __( 'Your licence for <strong>%s</strong> has been activated. Thanks!', 'wp-job-manager-visibility' ), esc_html( $this->plugin_data[ 'Name' ] ) ); ?></p>
		</div><?php
	}
	...
	public function admin_init() {
		...
		global $wp_version, $sMyles_JMV_Updater_runonce;
		...
		if ( empty( $this->instance_id ) ) {
			...
			if ( ! class_exists( 'sMyles_JMV_Update_Password_Management' ) ) {
				include_once( 'class-smyles-update-passwords.php' );
			}
			$smyles_gen_instance = new sMyles_JMV_Update_Password_Management();
		...
		if ( ! empty( $_POST[ $this->plugin_slug . '_licence_key' ] ) ) {
			...
			try {
				...
				if ( ! class_exists( 'sMyles_JMV_Update_Key_API' ) ) {
					include_once( 'class-smyles-update-key-api.php' );
				}

				$activate_results = json_decode( sMyles_JMV_Update_Key_API::activate( array(
					                                                                  'email'            => $email,
		...
		if ( ! $sMyles_JMV_Updater_runonce ) {
			add_action( 'admin_print_styles-plugins.php', array( $this, 'styles' ) );
			$sMyles_JMV_Updater_runonce = true;
		}
	...
	public function deactivate_licence() {
		...
		$reset = sMyles_JMV_Update_Key_API::deactivate( array(
			                                            'product_id'  => $this->plugin_product_id,
	...
	public function plugin_information( $args ) {
		...
		$response = maybe_unserialize( $response_body );
	...
	public function handle_errors( $errors ) {
		...
		if ( ! empty( $errors[ 'no_key' ] ) ) {
			...
			$this->add_error( sprintf( __( 'A licence key for %s could not be found. Maybe you forgot to enter a licence key when setting up %s.', 'wp-job-manager-visibility' ), esc_html( $this->plugin_data[ 'Name' ] ), esc_html( $this->plugin_data[ 'Name' ] ) ) );

		} elseif ( ! empty( $errors[ 'invalid_request' ] ) ) {

			$this->add_error( __( 'Invalid update request', 'wp-job-manager-visibility' ) );
	...
	public function deactivated_key_notice() {
		...
		?>
		<div class="updated">
		<p><?php printf( __( 'Your licence for <strong>%s</strong> has been deactivated.', 'wp-job-manager-visibility' ), esc_html( $this->plugin_data[ 'Name' ] ) ); ?></p>
		</div><?php
	}
	...
	public function deactivation() {
		...
		if( ! get_option('jmv_disable_deactivate_license') ) $this->deactivate_licence();
	}
		...
		<tr id="<?php echo esc_attr( $this->plugin_slug ); ?>_licence_key_row" class="active update plugin-update-tr smyles-update-licence-key-tr">
			<td class="plugin-update" colspan="100%">
					<div class="smyles-update-licence-key">
						<label for="<?php echo sanitize_title( $this->plugin_slug ); ?>_licence_key"><?php _e( 'Licence', 'wp-job-manager-visibility' ); ?>:</label>
						...
						<input type="email" id="<?php echo sanitize_title( $this->plugin_slug ); ?>_email" name="<?php echo esc_attr( $this->plugin_slug ); ?>_email" placeholder="Email address" value=""/>
						<span class="description"><?php printf( __( 'Enter your licence key and email and hit return. A valid key is required for automatic updates.  The key <strong>and email MUST</strong> match the details you can find on your <a href="%s" target="_blank">My Account</a> page.', 'wp-job-manager-visibility' ), 'https://plugins.smyl.es/my-account/' ); ?></span>
					</div>
				</td>
				<script>
					jQuery(
	...
	public function key_notice() {
		...
		?>
		<div class="updated">
			<p class="smyles-updater-dismiss" style="float:right;"><a href="<?php echo esc_url( add_query_arg( 'dismiss-' . sanitize_title( $this->plugin_slug ), '1' ) ); ?>"><?php _e( 'Hide notice', 'wp-job-manager-visibility' ); ?></a></p>
			<p><?php printf( __( '<a href="%s">Please activate your licence key</a> to get updates for <strong>%s</strong>.  This ensures you have the latest features, bug fixes, and experience!', 'wp-job-manager-visibility' ), esc_url( admin_url( 'plugins.php#' . sanitize_title( $this->plugin_slug ) ) ), esc_html( $this->plugin_data[ 'Name' ] ) ); ?></p>
			<p><small class="description"><?php printf( __( 'Lost your key? <a href="%s">Retrieve it here</a>.', 'wp-job-manager-visibility' ), esc_url( 'https://smyl.es/lost-licence-key/' ) ); ?></small></p>
		</div><?php
	}
